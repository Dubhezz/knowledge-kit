...
1. 打包机暂时不支持各个依赖 SDK 静态库的方式引入，根本原因是目前 APM 监控系统不支持。因为多个静态库则会生成多个 DYSM 文件，这样子 APM 定位 Crash、ANR 等都会生成多个 DYSM 文件的信息，配套的后端在处理的时候目前是只支持一个 DYSM 的模式，所以在如此背景下打包机不支持静态库。
2. 
...
1. New Build System 是否可以让单包构建变快？
   ...
   1.3.1 Legacy Build System 构建时间为  335.5s.
...
3. 升级 cocoapods 到 1.8.0，查看在 New Build System 和 Legacy Build System 下的构建时间
   ...
   New Build System: 368.7s
   Legacy Build System: 326.9s
4. 通过上述几组对比实验发现： **不同版本的 cocoapods， New Build System 的速度比如 Legacy Build System。**
   听到某种说法是 New Build System 本身会提高构建速度，但是搭配了 Cocoapods 由于资源「Copy Pods Resources」的问题反而会导致编译速度降低。
  
实验数据如下：

| App| 构建系统 |  Cocoapods 版本  | Build 结果 | 编译时间 |
|:-:|:-:|:-:|:-:|:-:|:-:|
| 挖财宝 | New Build System | 失败 | 1.3.1 | ~ |
| 挖财宝 | Legacy Build System | 成功| 1.3.1 | 335.4s |
| 挖财宝 | New Build System | 失败 | 1.4.0 | 383.5s |
| 挖财宝 | Legacy Build System | 成功| 1.4.1 | 302.9s |
| 挖财宝 | New Build System | 成功 | 1.8.4 |  s|
| 挖财宝 | Legacy Build System | 成功| 1.8.4 | 262.2s |

经实验数据可以看出：挖财宝 App，模拟打包机环境（cocoapods 1.3.1 + Legayc Build System）和新的打包环境 （cocoapods 1.8.4 + New Build System）,时间节省 `(354.0-221.7)/354.0 = 37%`

结论： 升级 cocoapods 版本，依旧使用 Legacy Build System 构建系统（听到一种说法是「新的构建系统反而慢的原因是和 cocoapods 不兼容」）
