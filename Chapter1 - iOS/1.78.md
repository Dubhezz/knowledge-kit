...
之前验证结果的有问题，本地 cocoapods 版本为 1.4.0，打包机环境为 1.3.1，所以方案评估有问题。这几天花时间做了对比实验，数据如下
...
4. 通过上述几组对比实验发现： **不同版本的 cocoapods， New Build System 的速度比 Legacy Build System 慢。**
...
| 挖财宝 | New Build System | 成功 | 1.8.4 |  324.4s|
| 挖财宝 | Legacy Build System | 成功| 1.8.4 | 262.2s |


结论： 从实验数据来看， New Build System 并不能单包加速。所以 New Build System 不做了。cocoapods 1.8.4 升级后会带来多包构建的能力，从实验数据来看也可以提高单包构建的速度。所以后期会升级 cocoapods ，自建 CDN。目前在做通跳、授权的授权，CDN 随后验证。
