...
之前验证结果和验证方向的有问题，本地 cocoapods 版本为 1.4.0，打包机环境为 1.3.1，所以方案评估有问题。这几天花时间做了对比实验，数据如下
...
2. 自建 CDN。目前在做通跳、授权的授权，自建 CDN 能力随后验证。预估是客户端先做，涉及到 wax、服务端的工作，暂无排期

拿自己的电脑部署脚本，当作本地打包机；拿挖财宝 App 打包，指定打包机为自己的电脑
