...
在移动互联网时代，对于每个公司、企业来说，用户的数据非常重要。重要到什么程度，用户在这个页面停留多久、点击了什么按钮、浏览了什么内容、什么手机、什么网络环境、App什么版本等都需要清清楚楚。
...
- 代码手动埋点：根据业务需求（运营、产品、开发多个角度出发）在需要埋点地方手动调用埋点接口上传埋点数据国内的第三方数据统计服务商就是采用这种策略。

- 可视化埋点：通过可视化配置工具完成采集节点，在前端自动解析配置并上报埋点数据，从而实现可视化“无痕埋点”
- 无痕埋点：通过技术手段，完成对用户的操作事件完成无差别的统计上传的工作，后期数据分析处理的时候通过技术手段筛选出合适的数据进行统计分析。
...
1. 代码手动埋点
   ...
   代码手动埋点的缺点就是你如果需要埋点，那么势必需要在工程代码的零零碎碎的地方去写埋点代码，埋点的成本较高。后期在做维护的时候由于埋点的代码和业务代码揉在一起，所以维护成本较高，违背了单一原则。

   比如你需要知道用户在点击购买按钮的时候上传用户信息，那么就在该按钮的点击事件里面去写埋点统计的代码，这样很显然的一个问题就是在之前业务逻辑的代码上面又多出了埋点的代码。问题显然就是埋点的工作量很大、代码维护成本较高、后期重构很头痛

   假如页面有 H5 的成分，那么 H5 页面变动的时候，你的埋点代码不去更新的话，变动的 H5 的业务就不方便埋点统计。
...
2. 可视化埋点
   ...
   **可视化埋点的出现，是为更好的解决代码埋点流程复杂、成本高、新开发的页面（H5、或者服务端下发的 json 去生成相应页面）不能及时拥有埋点能力**

   前端可视化的方式去配置哪些按钮、哪些操作需要埋点，勾选完成会生成一个配置文件，之后打开 App 或者网页就去下发这个配置文件，开发者通过技术手段将从服务端获取的关键数据塞到前端的 UI 控件上（举个例子 iOS 端的话 UIView 有个属性可以去设置我们从服务端获取的埋点数据）
...
那么这个对应关系是什么？我们需要唯一定位一个前端元素，那么想到的办法就是不管 Native 和 Web 前端，控件或者元素来说就是一个树形层级，DOM tree 或者 UI tree，所以我们通过技术手段定位到这个元素，以 Native  iOS 为例子假如我点击商品详情页的加入购物车按钮会根据 UI 层级结构生成一个唯一标识 “addCartButton.GoodsView.GoodsViewController” 。但是用户在使用 App 的时候去上传这串东西到服务端，服务端数据库存储这串很长的东西不是很好，所以会将这串东西生成一个 md5 值，类似于从元素唯一标识到 md5 值的映射关系上传。下次用户点击后上传的就是一个 md5 值。
...
1. 如何实现（以 Native iOS 端为例子）- 「不侵入业务代码」

   我们会想到 **AOP（Aspect Oriented Programming）**面向切面编程，动态的在函数调用前后插入相应的代码，在 Objective-C 中我们可以利用 Runtime 特性的 Method Swizzling 来 hook 相应的函数

   可以给 NSObject 添加个 Category。 NSObject+MethodSwizzling
...
2. 如何实现（以 Native iOS 端为例子）- 「全量收集」

   我们会想到 hook 几乎所有的App 代理方法、页面控制器生命周期方法、按钮点击事件、手势事件、各种系统控件的点击回调方法、应用状态切换等等。
...
3. 如何实现（以 Native iOS 端为例子）- 「如何唯一标识一个控件元素」
   ...
   但是有个问题，比如我们点击的元素是 UITableViewCell，那么它虽然可以定位到类似于这个标示 xxApp.GoodsViewController.GoodsTableView.GoodsCell，同类型的 Cell 有多个，所以单凭借这个字符串是没有办法定位具体的那个 Cell 被点击了。

   有2个解决方案：利用系统提供的 **accessibilityIdentifier** 官方给出的解释是标识用户界面元素的字符串。另一个方案是结合当前的元素遍历当前元素的父级元素的子元素，如果出现相同的元素，则需要判断当前元素是所在层级的第几个元素
...
思路是这样的，用户产生的数据会先保存到 AppMonitor 的内存中去，设置一个临界值（memoryEventMax = 50），对外部暴露出一个接口，用来将产生的数据往数据中心去存，如果存储的值达到设置的临界值 memoryEventMax，那么将内存中的数据写入文件系统，以 zip 的形式保存下来然后上传到埋点系统。如果没有达到临界值但是存在一些 App 状态切换的情况，这时候需要及时保存数据到持久化。当下次打开 App 就去从本地持久化的地方读取是否有未上传的数据，如果有就上传日志信息，成功后删除本地的日志压缩包。
...
![绑定页面唯一标识与功能描述的对应关系](./../assets/2019-03-07-UserTracet1.PNG)
