...
- 找出当前元素在父层同类型元素中的索引。根据当前的元素遍历当前元素的父级元素的子元素，如果出现相同的元素，则需要判断当前元素是所在层级的第几个元素
  ...
  //    if (self.lbp_identifier_ka == nil) {
          if ([self isKindOfClass:[UIView class]]) {
              ...
              self.lbp_identifier_ka = [self md5String:[NSString stringWithFormat:@"%@",str]];
              //            self.lbp_identifier_ka = [NSString stringWithFormat:@"%@",str];
          }
  //    }
      return self.lbp_identifier_ka;
  }
...
- (NSString *)setUniqueIdentifier
{
    if ([self.tagString isEqualToString:ImmediateyPurchase]) {
        return @"ImmediateyPurchase";
    } else if ([self.tagString isEqualToString:ShareToAward]) {
        return @"ShareToAward";
    } else {
